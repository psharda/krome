...
for arg in sys.argv:
	if("cooling=" in arg):
		...
		allCools = ["CEN","H2","H2GP98","HD","Z","ALL"]
		for coo in myCools: 
...
if(useDust):
	...
	for dType in dustTypes:
		for i in range(dustArraySize):
			...
			dns[nmols+j-1] += ",krome_dust_T("+str(j)+"),vgas,krome_dust_asize("+str(j)+"))"#!dust"+dType+str(i+1) 
			#******sputtering Nozawa2006*******
			#if(dType=="C"): zdust = 12
			#elif(dType=="Si"): zdust = 28
			#elif(dType=="Fe"): zdust = 56
			#else: die("ERROR: dust type "+dType+" unknown for sputtering model!")
			##find colliders (ions)
			#zcoll = []
			#ncoll = []
			#for mol in specs:
			#	if(mol.charge==1 and mol.is_atom): 
			#		zcoll.append(str(mol.zatom))
			#		ncoll.append("n("+mol.fidx+")")
			#dns[nmols+j-1] += " - krome_dust_sputtering(vgas,(/"+(",".join(zcoll))+"/),"
			#dns[nmols+j-1] += str(zdust)+",(/"+(",".join(ncoll))+"/),n("+str(nmols+j)+"),krome_dust_asize("+str(j)+"),Tgas)"
