...
#KROME_load_parts
  ...
  !H2,H,,H,H,H,,NONE,NONE,dissH2H_Hunter23(n,Tgas)
  function dissH2H_Hunter23(n,Tgas)
    use krome_commons
    use krome_getphys
    integer::i
    real*8::n(nspec),Tgas,dissH2H_Hunter23,n_H,xH,xH2
    ...
    dissH2H_Hunter23 = 1d1**logkH

  end function dissH2H_Hunter23


  !***************************
  !Collisional dissociation rate H2+H2->H+H+H2
  !Combination of MacLow & Shull 1986 for low densities
  !and Martin et al. 1996 for high densities
  !with a ramp between the two based on the critical density calculation
  !presented in Glover & MacLow 2007. However Glover & MacLow 2007, use a 
  ...
  !inclde He, which is a TODO for the future if needed
  !If you use this rate, it must be included in the reaction file as
  !H2,H2,,H,H,H,,NONE,NONE,dissH2H2_Hunter23(n,Tgas)
  function dissH2H2_Hunter23(n,Tgas)
    use krome_commons
    use krome_getphys
    integer::i
    real*8::n(nspec),Tgas,dissH2H2_Hunter23,n_H,xH,xH2
    real*8::n_cr_H,n_cr_H2,ncr,k22_h,k22_l,logkH
    real*8::k_CIDm(21,2)
    real*8::logk_h1a,logk_h2a,logk_h1b,logk_h2b
    real*8::logT,logT2,logT3,logT4,invT,logTv(4),T4
    ...
    !Low density limit from MacLow and Shull 1986
    !Actual equation from Gong et al. 2017, table 1
    k22_l = (5.996d-30*(Tgas**4.1881d0) / (1d0 + 6.761d-6*Tgas)**5.6881d0)*exp(-54657.4d0/Tgas)
    ...
    logk_h1b = k_CIDm(1,i)*logTv(1) + k_CIDm(2,i)*logTv(2) + &
          ...
          k_CIDm(5,i)*log10(1.d0+k_CIDm(6,i)*invT)
    logk_h2b = k_CIDm(7,i)*invT
    !scale the final rate by (1/8), as suggested by Jacobs et al. 1967 and done in Hunter et al. 2023
    k22_h = (1d1**(logk_h1a+logk_h2a) + 1d1**(logk_h1b+logk_h2b))/8d0

    ! Interpolation between low- and high-density limits
    logkH = (n_H / ncr) / (1d0 + (n_H / ncr)) * log10(k22_h) + (1d0 / (1d0 + (n_H / ncr))) * log10(k22_l)

    dissH2H2_Hunter23 = 1d1**logkH

  end function dissH2H2_Hunter23
