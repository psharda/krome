...
program sedov
  ...
  integer::nsteps, istep, ix, ndump,i,ibuf,jbuf,j,cfex,tfex
  ...
  do istep = 1, nsteps
     ...
     do ix = 1, nx
        ...
        if(bfound) then
           ...
           if(ibuf==nbuf) then
              ibuf = nbuf/2
              xibuf(1:ibuf,:) = xibuf(ibuf+1:nbuf,:)
              xfbuf(1:ibuf,:) = xfbuf(ibuf+1:nbuf,:)
              Tibuf(1:ibuf) = Tibuf(ibuf+1:nbuf)
              Tfbuf(1:ibuf) = Tibuf(ibuf+1:nbuf)
              dtbuf(1:ibuf) = dtbuf(ibuf+1:nbuf)
           end if
        end if
     ...
     if (mod(istep,ndump).eq.0) then
        write(*,100) istep,t/spy,t/tmax*100,cfex,tfex
        do ix = 1, nx
...
!format
100 format('step:',i6,'; t:',1pe10.2,'; ',0pf6.1,'%',2I8)
