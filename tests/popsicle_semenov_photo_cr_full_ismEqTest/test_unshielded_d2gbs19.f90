...
program test_krome_eqbm
  ...
  real*8::Av,heats(krome_nheats),crate,NH,NHj,NH2,d2g
  ...
  do jz = 1,size(zs)*2
    ...
    !D2G from Bialy & Sternberg 2019 eq. 5, which follows Remy-Ruyer+2014
    if (zs(jz2) .gt. 0.2d0) then
      d2g = zs(jz2)
    else
      d2g = 0.2d0*(zs(jz2)/0.2d0)**3d0
    endif
