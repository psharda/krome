...
program test_krome_eqbm
  ...
  do jz = 1,size(zs)*2
    ...
    do column_bins = 1, 10000
      ...
      !Dissociation rates
      dissH2 = 5.70d-11*exp(-4.18*Av)*chi0*get_fshield_H2(NH2_cum,bfive)  !H2 dissociation rate accounting for self-shielding
      call krome_set_user_dissH2(dissH2)
      ionC = 3.5d-10*krome_get_user_is_metal()*chi0*exp(-3.76*Av)*get_fshield_C(NH2_cum,NC_cum)
      dissCO = 2.4d-10*krome_get_user_is_metal()*chi0*exp(-3.88*Av)*get_fshield_CO(NH2_cum,NCO_cum)
      ...
      chiFUV = chi0 * exp(-1.d-21 * d2g * Nshield)
      ...
      do i=1, rstep
        ...
        !Dissociation rates
        dissH2 = 5.70d-11*exp(-4.18*Av)*get_fshield_H2(NH2_cum,bfive)  !H2 dissociation rate accounting for self-shielding
        call krome_set_user_dissH2(dissH2)
        ionC = 3.5d-10*krome_get_user_is_metal()*chi0*exp(-3.76*Av)*get_fshield_C(NH2_cum,NC_cum)
        dissCO = 2.4d-10*krome_get_user_is_metal()*chi0*exp(-3.88*Av)*get_fshield_CO(NH2_cum,NCO_cum)
        call krome_set_user_ionC(ionC)
