...
NOTE: TIGRESS-NCR only tracks H, H2, C, C+, O, CO and e- (see README) -- it has
no He, Si, H3+, CH or OH, so those rows are simply absent from the comparison
plot below, unlike the full-network and GOW versions of this benchmark.
"""
...
def plot_RolligCombined(outfile='RolligCombined.pdf'):
    ...
    # TIGRESS-NCR has no He, Si, H3+, CH or OH -- only these rows are available
    panels = [
    ...
    fig, axs = plt.subplots(nrows=NROWS, ncols=NCOLS, figsize=(21.6, 30.24),
