...
program sedov
  ...
  real*8::xx(nsp)
  !NEW VARIABLES FOR DECOUPLING TEST SB2013
  real*8::edot,dt_cl,ttmin,tot
...
222 format(a20,E11.3)
  ...
  do istep = 1, nsteps
     ...
     !TEMPERATURE DECOUPLED FROM THE SOLVER
     ...
     do iter=1,itmax
        do ix = 1, nx
           xx(:) = x(ix,:)
           rhogas = rho(ix)
           call krome_evaluate_edot(xx(:),Tgas(ix),rhogas,edot)
           !evaluate timestep keeping the e/de variation < 10%
           tot = ttot(ix)
           call krome_timestep(eps(ix),Tgas(ix),edot,dt,tot,dt_cl)
           dt_cool(ix)=dt_cl
           !update internal energy with new temperature
           eps(ix) = eps(ix) + edot/rhogas * dt_cool(ix)
           Tgas(ix) = (gamma - 1.d0) * eps(ix) / boltzmann_erg * p_mass
           call krome(xx(:),rhogas,Tgas(ix),dt_cool(ix))
           x(ix,:) = xx(:)
        enddo
        call krome_timestep_exit(nx,ttot,dt,dt_cool,ttmin)
        !If all cells are done, then exit
        if (abs(dt-ttmin) .lt. tolerance*dt) exit
     enddo
     !*********END******************************!
