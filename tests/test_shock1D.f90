...
program sedov
  ...
  !open the file in which values of thermal, kinetic and energy will be stored at 10th time-step
  open(17,file='lh2.out',form='formatted',status='replace')

  call krome_init()

  gamma = 5.d0/3.d0
  nsteps = int(1e3) !maximum number of time-steps
  ndump = 10 !dump interval
  Tgas(:) = 1d3 !default gas temp (K)  
  tmax = spy * 1d5 !maximum integration time (s)
