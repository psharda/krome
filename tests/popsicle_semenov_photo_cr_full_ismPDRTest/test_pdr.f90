...
program test_krome_eqbm
  ...
  real*8::Av,heats(krome_nheats),crate,crate_0,NH_cum,NH2_cum,NHD_cum,NC_cum,NCO_cum
  real*8::ionH,dissH2,dissHD,ionC,dissCO,chiFUV,chiLW,chiPE,chi0
  logical::stop_next, converged, first_call
  character(len=20) :: filename, zint_str
  real*8, parameter :: Lshield_0 = 1.5428d19, a = 0.7, n_0 = 1d2, sigmaD_LW = 1.5d-21, sigmaD_PE = 0.86d-21, bfive = 3d0 !we set bfive=3d0 for this test to compare with GOW 2017 (see text below equation 7)
  ...
  do jz = 1,size(zs)*2
    ...
    ColumnTotMin = 1d17/zs(jz2)   ! Minimum column density    
    ColumnTotMax = 1d22/zs(jz2)   ! Maximum column density
    ...
    do column_bins = 1, 10000
      ...
      if (first_call) then
        ...
        x(KROME_idx_E)         = 1.6d-4*zs(jz2)*ntot + 1d-4*ntot + 2.681411e-07*ntot !C+, H+ and H3+ contribute electrons
      ...
      do i=1, rstep
        ...
        dissHD = 5.60d-11*chiLW*get_fshield_H2(NHD_cum,bfive)
      ...
      write(31,'(99E14.5e3)') ColumnTot, Tgas, sum(cools), cools(:)
      kk(:) = krome_get_coef(Tgas,n(1:krome_nmols))
      heats(:) = get_heating_array(n(:),Tgas,kk(:),0d0)
      write(911,'(99E14.5e3)') ColumnTot, Tgas, sum(heats), heats(:)
      ...
      write(22,'(99E17.8e3)') Hnuclei,rhogas,Tgas,Tdust,ColumnTot,n(1:krome_nmols)/Hnuclei,t_tot,t_cool,real(i)
      flush(22)
